...
markmap:
  height: 643
---
# CHAPTER 1: What Is Operations Research (OR)?

## Origins & Purpose
### Started in WWII Britain: scientific allocation of war resources
### Post-war: adapted to improve civilian efficiency & productivity
### OR as a decision-making discipline: science + art
...
### Iterative/algorithmic computation

## 1.1 Introduction: Key Messages
### Correct problem definition is hardest and most important
### Models are essential, but unquantifiable factors (human behavior) matter
### Learning via solved examples, problems, and case analyses

## 1.2 Operations Research Models

### OR model components
#### Alternatives (choices)
#### Objective criterion (what “best” means)
#### Constraints (restrictions)

### Example A: Airline tickets (finite alternatives)
#### Setup: 5-week travel FYV ↔ DEN (Mon out, Wed back)
#### Costs
##### Regular roundtrip: $400
##### Weekend-spanning roundtrip: 20% discount
##### One-way: 75% of regular
#### Alternatives & costs (compare totals)
##### Alt 1: 5 regular roundtrips → $2000
##### Alt 2: mix of one-ways + weekend trips → $1880
##### Alt 3: all weekend-spanning roundtrips → $1600 (optimal)

### Example B: Garden fencing (continuous alternatives)
#### Fence length: $L = 100\ \mathrm{ft}$
#### Variables: $w$ (width), $h$ (height)
#### Constraint (perimeter): $2(w+h)=L$, with $w \ge 0,\ h \ge 0$
#### Objective: maximize area $z = wh$
#### [[#^qwer|Eliminate variable]]
##### $w = \frac{L}{2} - h$
##### $z(h) = \left(\frac{L}{2}-h\right)h = \frac{Lh}{2} - h^2$
#### Best solution (via calculus): $w=h=\frac{L}{4}=25\ \mathrm{ft}$ (square)
...
### Model completeness matters
#### “Optimal” is only optimal relative to modeled alternatives & assumptions
#### Missing key alternatives can make solutions suboptimal in reality

## 1.3 Solving the OR Model
### No single universal method; depends on model type/complexity
...
#### Solutions often not closed-form
#### Use iterative algorithms; computers handle large/tedious computations
### When optimization is too hard
#### Use heuristics/metaheuristics for good (not necessarily optimal) solutions

## 1.4 Queuing and Simulation Models
### Focus: waiting lines (performance measurement, not optimization)
#### Average waiting time in queue
#### Average service waiting time
#### Utilization of service facilities
### Queuing vs simulation
#### Queuing: mathematical/probabilistic, assumption-limited
#### Simulation: imitates system behavior; flexible for many scenarios
### Simulation drawback
#### Costly to build; slow to run even on fast computers

## Aha! Moment: Ada Lovelace & Algorithms
### “Algorithm” roots linked to Al-Khwarizmi (origin of the term)
### Ada Lovelace wrote a pioneering algorithm (Bernoulli numbers)
### Babbage’s Engines (Difference/Analytical) anticipated modern components
### Legacy: Ada language; Ada Lovelace Day (STEM celebration)

## 1.5 Art of Modeling (Abstraction)
### Exact models are rare; most are approximations
### Model development as abstraction layers
#### Real world → assumed real world (dominant variables)
#### Assumed real world → mathematical model (amenable functions)
### Tyko Manufacturing example
...
## 1.6 More Than Just Mathematics (Human Factors)
### Start with simplest approaches; psychology often decisive
### Illustrations
#### UPS ORION: “Beat the Computer” framing improved adoption
#### Houston airport: longer walk reduced baggage complaints
#### British airport: cultural norms broke a mathematically sound queue policy
#### Steel mill: simple charts revealed shift behavior; schedule leveling fixed it
#### Elevator: mirrors reduced perceived waiting
#### Shared trucks: visibility/communication solved “lack of trucks” complaint
### Four conclusions
#### Use different ideas; include social science/psychology
#### Take a bird’s-eye view to find nontechnical root causes
#### Don’t bias toward a favorite tool (e.g., LP) without justification
#### Solutions must fit people/culture, not just technology

## 1.7 Phases of an OR Study
### Teamwork: analysts + client collaborate
### OR as science (methods) + art (creativity, judgment, communication)
### Five phases
#### 1) Define the problem (alternatives, objective, limitations)
#### 2) Construct the model (relationships; may simplify)
#### 3) Solve the model (algorithms)
##### Sensitivity analysis: robustness to parameter changes
#### 4) Validate the model (sanity checks + compare to historical data)
#### 5) Implement the solution (operational instructions & adoption)

## 1.8 About This Book
### Teaching models ≠ teaching modeling (emphasis on realistic cases)
### Computation & software tools
#### Commercial packages (Excel/Solver, AMPL) + tutorial tools (e.g., TORA)
### Learning by practical cases
#### End-of-chapter case analyses + many real-world cases (including online)
#### Interfaces (INFORMS) as a rich OR application source
...
To demonstrate how the garden problem is expressed mathematically in terms of its two unknowns, width and height, define
...
Actually, this model can be simplified further by eliminating one of the variables in the objective function using the constraint equation; that is,
...
The maximization of $z$ is achieved by using differential calculus (Chapter 20), which yields the best solution as $h = \frac{L}{4} = {25}\mathrm{{ft}}$ . Back substitution in the constraint equation then yields $w = \frac{L}{4} = {25}\mathrm{{ft}}$ . Thus the solution calls for constructing a square-shaped garden.
...
In practice, OR does not offer a single general technique for solving all mathematical models. Instead, the type and complexity of the mathematical model dictate the nature of the solution method. For example, in Section 1.2 the solution of the tickets purchasing problem requires simple ranking of alternatives based on the total purchasing price, whereas the solution of the garden problem utilizes differential calculus to determine the maximum area.
...
A peculiarity of most OR techniques is that solutions are not generally obtained in (formula-like) closed forms. Instead, they are determined by algorithms. An [[algorithm]] provides fixed computational rules that are applied repetitively to the problem, with each repetition (called iteration) attempting to move the solution closer to the optimum. Because the computations in each iteration are typically tedious and voluminous, it is imperative in practice to use the computer to carry out these algorithms.

Some mathematical models may be so complex that it becomes impossible to solve them by any of the available optimization algorithms. In such cases, it may be necessary to abandon the search for the optimal solution and simply seek a good solution using heuristics or metaheuristics, a collection of intelligent search rules of thumb that move the solution point advantageously toward the optimum.
...
The use of simulation is not without drawbacks. The process of developing simulation models is costly in both time and resources. Moreover, the execution of simulation models, even on the fastest computer, is usually slow.
...
Because of the mathematical nature of OR models, one tends to think that an OR study is always rooted in mathematical analysis. Though mathematical modeling is a cornerstone of OR, simpler approaches should be explored first. In some cases, a "commonsense" solution may be reached through simple observations. Indeed, since the human element invariably affects most decision problems, a study of the psychology of people may be key to solving the problem. Six illustrations are presented here to demonstrate the validity of this argument.
...
OR studies are rooted in teamwork, where the OR analysts and the client work side by side. The OR analysts' expertise in modeling is complemented by the experience and cooperation of the client for whom the study is being carried out.
